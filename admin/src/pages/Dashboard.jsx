...
export default function Dashboard({ setAuth }) {
  ...
  const fetchData = async () => {
    try {
      const [projRes, expRes, skillRes] = await Promise.all([
        axios.get(`${import.meta.env.DEV ? 'http://localhost:5000' : 'https://manav-s-portfolio-63db.vercel.app'}/api/public/projects`),
        axios.get(`${import.meta.env.DEV ? 'http://localhost:5000' : 'https://manav-s-portfolio-63db.vercel.app'}/api/public/experience`),
  ...
  const deleteProject = async (id) => {
    ...
    try {
      ...
      await axios.delete(`${import.meta.env.DEV ? 'http://localhost:5000' : 'https://manav-s-portfolio-63db.vercel.app'}/api/admin/projects/${id}`, {
  ...
  const deleteExperience = async (id) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      await axios.delete(`${import.meta.env.DEV ? 'http://localhost:5000' : 'https://manav-s-portfolio-63db.vercel.app'}/api/admin/experience/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (err) {
  ...
  const deleteSkill = async (id) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      await axios.delete(`${import.meta.env.DEV ? 'http://localhost:5000' : 'https://manav-s-portfolio-63db.vercel.app'}/api/admin/skills/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchData();
    } catch (err) {
